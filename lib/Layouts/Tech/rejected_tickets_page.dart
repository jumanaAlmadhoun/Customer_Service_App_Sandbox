...
class RejectedTicketsPage extends StatefulWidget {
  const RejectedTicketsPage({Key? key}) : super(key: key);

  @override
  _RejectedTicketsPageState createState() => _RejectedTicketsPageState();
}

class _RejectedTicketsPageState extends State<RejectedTicketsPage>
    with RouteAware {
  ...
  void didPush() {
    ...
    Provider.of<TicketProvider>(context, listen: false)
        .fetchPendingTickets('$DB_TECH_REJECTED_TICKETS/$userName')
        .then((value) {
      setState(() {
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: ResponsiveWrapper.of(context).isSmallerThan(DESKTOP)
          ? AppBar(
              title: const Text('التذاكر المرفوضة'),
              actions: [
                IconButton(
                  onPressed: () {
                    setState(() {
