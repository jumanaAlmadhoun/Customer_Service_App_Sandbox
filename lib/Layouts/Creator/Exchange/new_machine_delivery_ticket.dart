...
class NewPickupTicket extends StatefulWidget {
  ...
  @override
  _NewPickupTicketPageState createState() => _NewPickupTicketPageState();
}

class _NewPickupTicketPageState extends State<NewPickupTicket> with RouteAware {
    ...
    if (_techName != NA) {
      if (formKey.currentState!.validate()) {
        return await Provider.of<TicketProvider>(context, listen: false)
            .submitNewPickupTicket(json, '$DB_URL$DB_EXCHANGE_TICKETS.json');
      }
    } else {
      return await Provider.of<TicketProvider>(context, listen: false)
          .submitNewPickupTicket(json, '$DB_URL$DB_EXCHANGE_TICKETS.json');
  ...
  Map<String, dynamic>? getTicketHeader() {
    return {
      Ticket.OUT_MACHINE: {
        ...
        Ticket.SUB_CATEGORY: Ticket.DELIVERY_CATEGORY,
